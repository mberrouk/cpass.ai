...
export default function TVETDashboardNewDjango() {
  ...
  const demoInstitutionCode = localStorage.getItem('demo_institution_code') || 'BUKURA';

  // Demo institution data
  const demoInstitutions: Record<string, any> = {
    'BUKURA': { 
      institution_name: 'Bukura Agricultural College', 
      location: 'Kakamega', 
      institution_code: 'BUKURA' 
    },
    'KSA': { 
      institution_name: 'Kenya School of Agriculture', 
      location: 'Nairobi', 
      institution_code: 'KSA' 
    },
    'BARAKA': { 
      institution_name: 'Baraka Agricultural College', 
      location: 'Molo', 
      institution_code: 'BARAKA' 
    },
  };
  ...
  const rplCount = isDemoMode ? 0 : (stats?.rpl_candidates || 0);
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {institutionLoading && !isDemoMode ? 'Loading...' : currentInstitution?.institution_name || 'TVET Institution'}
                  {isDemoMode && <span className="ml-2 text-xs bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded">DEMO</span>}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-4">
      ...
      <div className="max-w-7xl mx-auto px-6 py-6">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="bulk-upload" className="animate-fade-up">
            <BulkUploadTabDjango institutionCode={currentInstitution?.institution_code || ''} />
          </TabsContent>
        </Tabs>
      </div>
    </div>
