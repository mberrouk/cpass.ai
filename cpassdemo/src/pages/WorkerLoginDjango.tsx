...
// Demo profiles - use consistent password
...
export default function WorkerLoginDjango() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await djangoClient.auth.signInWithPassword({
        email,
        password,
      });
  ...
  const handleDemoProfileClick = async (profile: DemoProfile) => {
    ...
    let { error: signInError } = await djangoClient.auth.signInWithPassword({
      email: profile.email,
      password: DEMO_PASSWORD,
    });
    ...
    if (signInError && legacyPasswords[profile.email]) {
      const { error: legacyError } = await djangoClient.auth.signInWithPassword({
        email: profile.email,
    ...
    if (signInError) {
      ...
      const { error: signUpError } = await djangoClient.auth.signUp({
        email: profile.email,
        password: DEMO_PASSWORD,
        data: {
      ...
      if (signUpError && !signUpError.message?.includes("already")) {
        toast({
          ...
          description: signUpError.message || "Could not create demo account",
          variant: "destructive",
        });
        setLoading(false);
        return;
      ...
      const { error: retryError } = await djangoClient.auth.signInWithPassword({
        email: profile.email,
        password: DEMO_PASSWORD,
      });
  ...
  const handleSendOtp = async () => {
    ...
    try {
      const { error } = await djangoClient.auth.signInWithOtp({
        phone: `+254${phone}`,
      });
